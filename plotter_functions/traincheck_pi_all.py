"""
function to check pions with cuts to make them resemble photons
and to decide cuts to apply to training samples
"""
...
def traincheck_all(data_list) -> list:
    """
    function to select pions
    """
...
def plot_traincheck_all(data_list, out_dir) -> None:
    """
    ...
    LCtensor_list = traincheck_all(data_list)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, 'traincheck_pi_all.png')) #save plot
    plt.close(fig)
